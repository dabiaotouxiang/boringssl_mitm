...
## 编译
1. 编译动态库命令 `mkdir build && cd build && cmake -DBUILD_SHARED_LIBS=1 ../ && make && cd ../`
2. 将`include/openssl`拷贝至`/usr/local/include/`
3. 将`build/ssl/libssl.so build/crypto/libcrypto.so` 拷贝至 `/usr/local/lib/`
